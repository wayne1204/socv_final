...
void SATMgr::itpUbmc(const V3NetId& monitor, SatProofRes& pRes) {
  ...
  /// PART I:
   ...
   uint32_t k = 0;
   int clause_start = 0;
   satSolver->addBoundedVerifyData( I, k );
   satSolver->addBoundedVerifyData( monitor, k );
   satSolver->assumeRelease();
   satSolver->assumeProperty(I, false, k );
   satSolver->assumeProperty(monitor, false, k); 
   satSolver->simplify();
   
   // BMC_0 (S0, F) == SAT => reachable
   if(satSolver->assump_solve()) {
      pRes.setFired(k);
      return;
   }
   for(uint32_t j = 0; j < _ntk->getLatchSize(); ++j)
   {
      V3NetId net_id = _ntk->getLatch(j);
      satSolver->addBoundedVerifyData(net_id, k+1);   // important!
      Var var = satSolver->getVerifyData(net_id, k+1);
      mapVar2Net(var, net_id);
   }
   for(ClauseId cid = clause_start; cid < getNumClauses(); ++cid){
      markOnsetClause(cid); // A = S0 & C0
   }
   clause_start= getNumClauses();
  ...
  ++k;
   for(uint32_t max = pRes.getMaxDepth(); k< max; ++k){
      // cout <<"time " << k << endl;
      satSolver->addBoundedVerifyData(monitor, k);
      satSolver->assumeRelease();
      ...
      for(int i = 0; i < k; ++i){
         // cout << i << endl;
         V3NetId next_state = getItp();
         V3NetId next_reachable = ~_ntk->createNet();
         createAigAndGate(_ntk, next_reachable, ~reachable, ~next_state);
         satSolver->resizeNtkData(1);

         // if BMC_k (S_i+1, F) == SAT => break;
         satSolver->addBoundedVerifyData(next_state, 0);
         // satSolver->addBoundedVerifyData(monitor, k);
         satSolver->assumeRelease();
         satSolver->assumeProperty(next_state, false, 0);
         satSolver->assumeProperty(monitor, false, k);
         satSolver->simplify();
         ...
         // check converge (R_i == R_i+1)
         SatSolver* duplicate_solver = new SatSolver(_ntk);
         V3NetId tmp1 = _ntk->createNet();
         createAigAndGate(_ntk, tmp1, next_reachable, ~reachable);
         V3NetId tmp2 = _ntk->createNet();
         createAigAndGate(_ntk, tmp2, ~next_reachable, reachable);
         V3NetId eqv = ~_ntk->createNet();
         createAigAndGate(_ntk, eqv, ~tmp1, ~tmp2);
         satSolver->resizeNtkData(3);
         duplicate_solver->resizeNtkData(3);
         
         duplicate_solver->addBoundedVerifyData(eqv, 0);
         duplicate_solver->assumeRelease();
         duplicate_solver->assumeProperty(eqv, false, 0);
         duplicate_solver->simplify();
         bool result = duplicate_solver->assump_solve();
         delete duplicate_solver;
         if(!result){
            pRes.setProved(k);
            return;
         }
