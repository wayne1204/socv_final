...
void SatSolver::addBoundedVerifyData(const V3NetId& id, const uint32_t& depth)
{
    if( existVerifyData( id, depth) ) return;
    addBoundedVerifyDataRecursively(id,depth);
}

void SatSolver::addBoundedVerifyDataRecursively(const V3NetId& id, const uint32_t& depth)
{
    assert( validNetId(id) );  
    if( existVerifyData(id,depth) ) return;
