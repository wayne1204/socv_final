  ...
  while (!Q.empty()) {
    ...
    if (!isBlocked(s)) {
      ...
      if (z._frame != -1) {
        ...
        if (debug) {
          cout << "cube after generalized :";
          z._cube->show();
          cout << " frame : " << z._frame << endl;
        }
        ...
        Q.push(z);
        if (debug) {
          cerr << "pushed z: " << z._cube << ", ";
          z._cube->show();
        }
        Q.push(s); // put it in queue again
        if (debug) {
          cerr << "pushed s: " << s._cube << ", ";
          s._cube->show();
        }
      }
    }
...
TCube PDRMgr::generalize(TCube s) {
  ...
  for (unsigned i = 0; i < L; ++i) {
    ...
    if (tc->_latchValues[i].dontCare()) continue;
    else tc->_latchValues[i]._dont_bit |= 2;
